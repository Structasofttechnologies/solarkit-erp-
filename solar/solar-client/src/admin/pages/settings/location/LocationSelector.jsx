...
const LocationSelector = ({
    ...
    fetchParams = {} // e.g. { district: { unassignedToCluster: true } }
}) => {
    const levels = isZoneMode
        ? ['country', 'state', 'cluster', 'zone', 'district', 'city']
    ...
    const fetchDistricts = async (parentId, isCluster = false) => {
        ...
        try {
            ...
            const params = { isActive: true, ...(fetchParams.district || {}) };
    ...
    const fetchClusters = async (parentId, isState = false) => {
        ...
        try {
            ...
            const params = { isActive: true, ...(fetchParams.cluster || {}) };
    ...
    const handleUpdate = (level, val) => {
        ...
        if (!multiple[level]) {
            ...
            if (isZoneMode) {
                if (level === 'cluster') Object.assign(updates, { zone: multiple.zone ? [] : '', district: multiple.district ? [] : '', city: '' });
                if (level === 'zone') Object.assign(updates, { district: multiple.district ? [] : '', city: '' });
                if (level === 'district') Object.assign(updates, { city: '' });
            } else {
            ...
            return (
                <div className="flex-1 min-w-[200px]">
                    ...
                    <div className={`border rounded-lg p-2 max-h-40 overflow-y-auto ${isDisabled ? 'bg-gray-100' : 'bg-white'} ${errors[level] ? 'border-red-500' : 'border-gray-300'}`}>
                        ...
                        {currentOptions.length === 0 && <p className="text-gray-400 text-xs p-1">No options available</p>}
                    </div>
    ...
    return (
        <div className={layout === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4' : 'flex flex-col gap-4'}>
            ...
            {isZoneMode ? (
                <>
                    {renderSelect('cluster', data.clusters, 'Cluster', values.state)}
                    {renderSelect('zone', data.zones, 'Zone', values.cluster)}
                    {renderSelect('district', data.districts, 'District', showLevels.includes('zone') ? values.zone : values.cluster)}
                </>
            ) : (
                <>
            ...
            {renderSelect('city', data.cities, 'City', isZoneMode ? values.district : values.zone)}
        </div>
    );
};
