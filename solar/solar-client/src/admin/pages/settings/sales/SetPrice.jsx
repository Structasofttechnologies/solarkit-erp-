...
export default function SetPrice() {
  ...
  const handleEditExisting = (item) => {
    ...
    // 3. Sync Partner Type
    const pOptions = [...partnerTypes.map(p => p.name), 'All Partners'];
    const matchedPartner = pOptions.find(p => p.toLowerCase() === rawPartnerType.toLowerCase());
  ...
  return (
    <div className="bg-[#f4f7fa] min-h-screen font-sans">
      ...
      <div className="max-w-[1400px] mx-auto px-12 pb-20">
        ...
        <div className="space-y-6 mb-8">
            <div className="bg-white rounded-lg shadow-sm border border-gray-100 flex flex-col xl:flex-row p-0 overflow-hidden">
               ...
               <div className="xl:w-[350px] shrink-0 p-5">
                   <h5 className="font-bold text-[#14233c] mb-4 text-[15px]">Kit Type Filter</h5>
                   <select 
                      className="w-full px-3 py-2 border border-gray-200 rounded text-[13px] text-gray-700 bg-white focus:outline-none focus:border-[#0076a8] mb-4"
                      value={kitType} 
                      onChange={(e) => setKitType(e.target.value)}
                   >
                      <option value="All">All Kits</option>
                      <option value="Combo Kit">Combo Kits</option>
                      <option value="Custom Kit">Customized Kits</option>
                   </select>
        ...
        {selectedDistrictId && (
          <div className="bg-white shadow-sm border border-gray-100 rounded-lg overflow-hidden animate-in fade-in zoom-in-95 duration-500">
                ...
                <table className="w-full text-left border-collapse text-sm min-w-[1200px]">
                  <thead className="bg-[#343a40] text-white">
                    <tr>
                      ...
                      <th className="p-3 border-r border-white/20 text-center text-xs font-bold leading-tight">Kit Name</th>
                           ...
                           <tr key={row.id} className={`border-b border-gray-100 transition-all duration-300 ${row.marketPrice > row.benchmarkPrice ? 'bg-red-50 hover:bg-red-100/80 border-l-4 border-red-500' : 'hover:bg-gray-50'}`}>
                              ...
                              <td className="p-2 border-r border-gray-100 text-center">
                                  {row.isEditing ? (
                                    <input 
                                      type="number" 
                                      className="w-28 px-2 py-1.5 border border-blue-400 bg-white rounded-full text-center text-xs mx-auto focus:outline-none" 
             ...
             <div className="p-4 border-t border-gray-100 flex justify-between items-center">
                 ...
                 <button onClick={handleOpenAddModal} className="bg-[#0076a8] text-white px-5 py-2 rounded shadow text-sm font-bold flex items-center gap-2 hover:bg-blue-700 transition-colors">
        ...
        {summaryData.length === 0 && selectedPartnerType && selectedPartnerType !== 'all' ? (
          <div className="mt-16 bg-gray-50 border-2 border-dashed border-gray-300 rounded-xl p-10 text-center animate-in fade-in duration-500">
            <h3 className="text-xl font-bold text-gray-500 mb-2">No Prices Configured Yet</h3>
            <p className="text-gray-500 text-sm">There are no saved prices for this Partner Type and Plan. <br/>To set a price, select a <b>Kit Type</b>, <b>Brand</b>, and <b>Product Type</b> in the filters above.</p>
          </div>
        ) : summaryData.length > 0 && (
                ...
                <div><label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Benchmark Price</label><input type="number" className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-all text-sm" value={newPriceForm.benchmarkPrice} onChange={e => {
                 ...
                 <div>
                   <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">
                     Latest Buying Price (Market Price)
                   </label>
                   <input 
                     type="number" 
                     className="w-full px-4 py-3 border border-gray-200 rounded-lg outline-none text-sm" 
                     value={newPriceForm.marketPrice} 
                     onChange={e => {
                       const mkt = Number(e.target.value) || 0;
                       setNewPriceForm({ 
                         ...newPriceForm, 
                         marketPrice: mkt, 
                         margin: mkt - (newPriceForm.benchmarkPrice || 0) 
                       });
                     }}
                   />
                 </div>
