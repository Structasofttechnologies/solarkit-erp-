...
import { getPartnerProfessions, createPartnerProfession, deletePartnerProfession, getPartners, getPartnerPlans, getPartnerIndustryTypes, createPartnerIndustryType } from '../../../../services/partner/partnerApi';
...
export default function PartnerProfessionType() {
  ...
  useEffect(() => {
    if (selectedPartnerType) {
      fetchIndustryTypes(selectedPartnerType);
    } else {
      setIndustryTypes([]);
  ...
  const fetchIndustryTypes = async (type) => {
    try {
      const data = await getPartnerIndustryTypes(type);
      setIndustryTypes(data);
      if (data.length > 0) {
        setSelectedIndustryType(data[0]._id);
      } else {
        setSelectedIndustryType('');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (selectedPartnerType && selectedIndustryType) {
      fetchProfessions();
    } else {
      setProfessions([]);
    }
  }, [selectedPartnerType, selectedPlan, selectedIndustryType]);

  const fetchProfessions = async () => {
    try {
      setLoading(true);
      const data = await getPartnerProfessions(selectedPartnerType, undefined, selectedPlan, selectedIndustryType);
      setProfessions(data);
    } catch (error) {
      console.error(error);
  ...
  const handleAddProfession = async () => {
    ...
    if (!selectedIndustryType) {
      toast.error('Please select or create an industry type first');
      return;
    }
    try {
      const payload = {
      ...
      await createPartnerProfession(payload);
  ...
  const handleAddIndustryType = async () => {
    if (!newIndustryType.trim()) {
      toast.error('Please enter an industry type name');
      return;
    }
    try {
      const payload = {
        name: newIndustryType,
        partnerType: selectedPartnerType
      };
      const created = await createPartnerIndustryType(payload);
      toast.success('Industry type added');
      setNewIndustryType('');
      await fetchIndustryTypes(selectedPartnerType);
      setSelectedIndustryType(created._id);
    } catch (error) {
      console.error(error);
  ...
  const handleDeleteProfession = async (id) => {
    if (window.confirm('Delete this profession?')) {
      try {
        await deletePartnerProfession(id);
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-6 space-y-4">
        <div className="bg-white rounded-xl shadow-sm p-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold text-gray-800">Partner Profession Types</h1>
          </div>
        </div>

        {/* Partner Type & Plan Selector & Search */}
        <div className="bg-white rounded-xl shadow-sm p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
          ...
          <div className="w-full space-y-2">
            <label className="block text-sm font-medium text-gray-700">Select Industry Type</label>
            <select
              value={selectedIndustryType}
              onChange={(e) => setSelectedIndustryType(e.target.value)}
              className="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 border"
              disabled={!selectedPartnerType || industryTypes.length === 0}
            >
            ...
            <div className="flex gap-2">
              <input 
                ...
                className="flex-1 text-sm border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 border outline-none"
              />
              <button 
                onClick={handleAddIndustryType}
                disabled={!selectedPartnerType}
                className="bg-green-600 text-white p-2 rounded-lg hover:bg-green-700 transition-colors flex items-center justify-center disabled:opacity-50"
              >
          ...
          <div className="w-full">
            <label className="block text-sm font-medium text-gray-700 mb-1">Search State</label>
      ...
      {!selectedPartnerType || !selectedIndustryType ? (
        <div className="bg-white rounded-xl p-8 text-center text-gray-500 shadow-sm border border-dashed flex flex-col items-center">
          ...
          Please select a Partner Type and an Industry Type to configure professions.
        </div>
      ) : (
        <>
          {/* States Grid */}
