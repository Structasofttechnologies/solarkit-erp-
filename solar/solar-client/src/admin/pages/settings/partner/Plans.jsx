...
export default function PartnerPlans() {
    ...
    useEffect(() => {
        if (selectedCountryIds.length > 0) {
            const fetchStatesForCountry = async () => {
                try {
                    const ids = selectedCountryIds.includes('All') ? countries.map(c => c._id).join(',') : selectedCountryIds.join(',');
                    const data = await getStates({ countryId: ids });
                    setStates(data);
                    setSelectedStateIds([]);
    ...
    useEffect(() => {
        if (selectedStateIds.length > 0) {
            const fetchClustersForState = async () => {
                try {
                    const ids = selectedStateIds.includes('All') ? states.map(s => s._id).join(',') : selectedStateIds.join(',');
                    const data = await getClustersHierarchy(ids);
                    setClusters(data);
                    setSelectedClusterIds([]);
                    setSelectedDistrictIds([]);
                } catch (error) {
    ...
    useEffect(() => {
        if (selectedClusterIds.length > 0) {
            const fetchDistrictsForCluster = async () => {
                try {
                    const ids = selectedClusterIds.includes('All') ? clusters.map(c => c._id).join(',') : selectedClusterIds.join(',');
                    const data = await getDistrictsHierarchy(ids);
                    setDistricts(data);
                    setSelectedDistrictIds([]);
                } catch (error) {
    ...
    const getPlanCount = (category, value) => {
        ...
        if (category !== 'partnerType' && selectedPartnerTypes.length > 0 && !selectedPartnerTypes.includes('All')) {
            filtered = filtered.filter(p => p.partnerType?.some(t => selectedPartnerTypes.includes(t)));
        }
        if (category !== 'country' && selectedCountryIds.length > 0 && !selectedCountryIds.includes('All')) {
            filtered = filtered.filter(p => p.country?.some(id => selectedCountryIds.includes(id._id || id)));
        }
        if (category !== 'state' && selectedStateIds.length > 0 && !selectedStateIds.includes('All')) {
            filtered = filtered.filter(p => p.state?.some(id => selectedStateIds.includes(id._id || id)));
        }
        if (category !== 'cluster' && selectedClusterIds.length > 0 && !selectedClusterIds.includes('All')) {
            filtered = filtered.filter(p => p.cluster?.some(id => selectedClusterIds.includes(id._id || id)));
        }
        if (category !== 'district' && selectedDistrictIds.length > 0 && !selectedDistrictIds.includes('All')) {
    ...
    const fetchPlans = async () => {
        try {
            ...
            // Map 'All' to empty string so backend doesn't filter by it (which means fetch all)
            const pType = selectedPartnerTypes.includes('All') ? '' : selectedPartnerTypes.join(',');
            const cId = selectedCountryIds.includes('All') ? '' : selectedCountryIds.join(',');
            const sId = selectedStateIds.includes('All') ? '' : selectedStateIds.join(',');
            const clId = selectedClusterIds.includes('All') ? '' : selectedClusterIds.join(',');
            const dId = selectedDistrictIds.includes('All') ? '' : selectedDistrictIds.join(',');

            // Fetch both filtered and global plans
            const [filteredData, allPlansRes] = await Promise.all([
                getPartnerPlans(pType, sId, cId, clId, dId),
            ...
            <div className="mb-6 space-y-6">
                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-4">
                        {/* All Partners Card */}
                        <div
                            onClick={() => setSelectedPartnerTypes(['All'])}
                            className={`relative cursor-pointer px-6 py-4 rounded-xl shadow-sm text-center min-w-[150px] transition-all bg-white border ${selectedPartnerTypes.includes('All')
                                ? 'border-blue-500 text-blue-600 font-bold ring-1 ring-blue-500 bg-blue-50'
                                : 'border-gray-200 text-gray-600 hover:border-blue-300'
                                }`}
                        >
                            <div className="font-bold text-lg">All Partner Types</div>
                            <div className="text-gray-500 text-sm mt-1 uppercase tracking-wider">ALL</div>
                        </div>
                        ...
                        {partners.map((partner) => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`relative cursor-pointer px-6 py-4 rounded-xl shadow-sm text-center min-w-[150px] transition-all bg-white border ${isSelected
                                        ? 'border-blue-500 text-blue-600 font-bold ring-1 ring-blue-500 bg-blue-50'
                                        : 'border-gray-200 text-gray-600 hover:border-blue-300'
                                        }`}
                                >
                    ...
                    <div className="pt-2 space-y-6">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-4">
                                {/* All Countries Card */}
                                <div
                                    onClick={() => setSelectedCountryIds(['All'])}
                                    className={`relative cursor-pointer px-6 py-4 rounded-xl shadow-sm text-center min-w-[180px] transition-all bg-white border ${selectedCountryIds.includes('All')
                                        ? 'border-blue-500 ring-1 ring-blue-500 bg-blue-50'
                                        : 'border-gray-200 hover:border-blue-300'
                                        }`}
                                >
                                    <div className={`font-bold text-lg ${selectedCountryIds.includes('All') ? 'text-blue-600' : 'text-gray-800'}`}>
                                        All Countries
                                    </div>
                                    <div className="text-gray-500 text-sm mt-1 uppercase tracking-wider">ALL</div>
                                </div>
                                ...
                                {countries.map((country) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`relative cursor-pointer px-6 py-4 rounded-xl shadow-sm text-center min-w-[180px] transition-all bg-white border ${isSelected
                                                ? 'border-blue-500 ring-1 ring-blue-500 bg-blue-50'
                                                : 'border-gray-200 hover:border-blue-300'
                                                }`}
                                        >
                                ...
                                <div className="flex flex-wrap gap-4">
                                    {states.length > 0 ? (
                                        <>
                                            {/* All States Card */}
                                            <div
                                                onClick={() => setSelectedStateIds(['All'])}
                                                className={`relative cursor-pointer px-6 py-4 rounded-xl shadow-sm text-center min-w-[180px] transition-all bg-white border ${selectedStateIds.includes('All')
                                                    ? 'border-blue-500 ring-1 ring-blue-500 bg-blue-50'
                                                    : 'border-gray-200 hover:border-blue-300'
                                                    }`}
                                            >
                                                <div className={`font-bold text-lg ${selectedStateIds.includes('All') ? 'text-blue-600' : 'text-gray-800'}`}>
                                                    All States
                                                </div>
                                                <div className="text-gray-500 text-sm mt-1 uppercase">ALL</div>
                                            </div>
                                    ...
                                    <div className="flex flex-wrap gap-3">
                                        {clusters.length > 0 ? (
                                            <>
                                                {/* All Clusters Card */}
                                                <div
                                                    onClick={() => setSelectedClusterIds(['All'])}
                                                    className={`relative cursor-pointer px-6 py-3 rounded-xl shadow-sm text-center min-w-[160px] transition-all bg-white border ${selectedClusterIds.includes('All')
                                                        ? 'border-blue-500 ring-1 ring-blue-500 bg-blue-50'
                                                        : 'border-gray-200 hover:border-blue-300'
                                                        }`}
                                                >
                                                    <div className={`font-bold text-[15px] ${selectedClusterIds.includes('All') ? 'text-blue-600' : 'text-gray-800'}`}>
                                                        All Clusters
                                                    </div>
                                                    <div className="text-gray-500 text-xs mt-0.5 uppercase tracking-wide">ALL</div>
                                                </div>
                                                ...
                                                {clusters.map((cluster) => {
                                                    ...
                                                    return (
                                                        <div
                                                            ...
                                                            className={`relative cursor-pointer px-6 py-3 rounded-xl shadow-sm text-center min-w-[160px] transition-all bg-white border ${isSelected
                                                                ? 'border-blue-500 ring-1 ring-blue-500 bg-blue-50'
                                                                : 'border-gray-200 hover:border-blue-300'
                                                                }`}
                                                        >
                                    ...
                                    <div className="flex flex-wrap gap-3">
                                        {districts.length > 0 ? (
                                            <>
                                                {/* All Districts Card */}
                                                <div
                                                    onClick={() => setSelectedDistrictIds(['All'])}
                                                    className={`relative cursor-pointer px-6 py-3 rounded-xl shadow-sm text-center min-w-[160px] transition-all bg-white border ${selectedDistrictIds.includes('All')
                                                        ? 'border-blue-500 ring-1 ring-blue-500 bg-blue-50'
                                                        : 'border-gray-200 hover:border-blue-300'
                                                        }`}
                                                >
                                                    <div className={`font-bold text-[15px] ${selectedDistrictIds.includes('All') ? 'text-blue-600' : 'text-gray-800'}`}>
                                                        All Districts
                                                    </div>
                                                    <div className="text-gray-500 text-xs mt-0.5 uppercase tracking-wide">ALL</div>
                                                </div>
