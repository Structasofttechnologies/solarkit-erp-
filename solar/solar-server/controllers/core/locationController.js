...
export const getAllDistricts = async (req, res, next) => {
  try {
    const { stateId, countryId, clusterId, isActive, unassignedToCluster, editingClusterId } = req.query;
    const query = {};
    ...
    if (unassignedToCluster === 'true') {
      const clusterQuery = {};
      if (editingClusterId && editingClusterId !== 'undefined' && editingClusterId !== 'null') {
        clusterQuery._id = { $ne: editingClusterId };
      }
      const existingClusters = await Cluster.find(clusterQuery);
      const assignedDistrictIds = existingClusters.reduce((acc, c) => [...acc, ...c.districts.map(d => d.toString())], []);
      
      if (assignedDistrictIds.length > 0) {
        if (query._id && query._id.$in) {
          query._id.$in = query._id.$in.filter(id => !assignedDistrictIds.includes(id.toString()));
        } else {
          query._id = { ...query._id, $nin: assignedDistrictIds };
        }
      }
    }
