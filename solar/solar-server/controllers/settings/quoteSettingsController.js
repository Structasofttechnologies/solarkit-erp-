...
// Helper to filter out placeholder values like 'all' from mongoose object ID fields
const sanitizeQuotePayload = (body) => {
    const fields = ['countries', 'states', 'clusters', 'districts'];
    fields.forEach(field => {
        if (Array.isArray(body[field])) {
            body[field] = body[field].filter(id => id && id !== 'all' && id.toString().length === 24);
        }
    });
};
